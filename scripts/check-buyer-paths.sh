...
require_text 'Add to Chrome — free' "$page"
require_text 'Free Stores + Timeline · local state · no account required' "$page"
require_text '"softwareVersion": "1.2.0"' "$page"
require_text 'id="setup"' "$page"
require_text 'npm install zustand-devtools-bridge' "$page"
require_text 'extension 1.2.0 / bridge 0.2.1' "$page"
...
require_text 'Unlock Pro on Gumroad' "$page"
require_text '+ applicable tax' "$page"
require_text 'Interactive walkthrough · simulated panel' "$page"
require_text "'counter/increment'" "$page"
require_text 'React is not required for registered stores' "$page"
require_text 'Time travel restores captured store state in this tab' "$page"
require_text 'id="diagnosis"' "$page"
require_text 'September clinic · intake closed' "$page"
require_text 'intake closed' "$page"
require_text 'Ask about a future clinic' "$page"
...
require_text 'Check diagnosis options' "$repo_root/debug-zustand-state-changes.html"
...
reject_text 'follow every action' "$page"
reject_text 'no setup or account' "$repo_root/try-zustand.html"
